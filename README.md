...
This package is prepared to run on Railway with a Node 20 Docker runtime.
1. Deploy this folder/repository as the service root.
2. Railway will detect `Dockerfile` and build the Node 20 image.
3. The container starts with `node start.js`, which runs both the web status server and the WhatsApp bot.
4. The web server listens on Railway's `PORT` and binds to `0.0.0.0`.
5. Do not override the Start Command with `npm start` in a non-Node runtime.

The Railway-specific fix is the Dockerfile + `start.js` entrypoint. This prevents the previous
`/bin/bash: npm: command not found` runtime failure caused by a non-Node/Caddy runtime.


## Railway build fix

This package is prepared for Railway's Docker builder. The Dockerfile installs Git before
`npm install` because Baileys 6.7.24 uses a libsignal dependency that can be fetched through
Git. It also rewrites GitHub git/SSH URLs to HTTPS for more reliable cloud builds.

Railway:
- Builder: Dockerfile
- Start command: `node start.js`
- Node: 20 (provided by Dockerfile)

## Public Multi-Pairing
- Open the Railway public domain and enter a WhatsApp number with country code.
- Approve the request in WhatsApp: Linked devices -> Link a device -> Link with phone number instead.
- Each approved number gets its own isolated session.
- `BOT_MODE=public` allows normal commands for members; admin-only commands remain protected.
- `MAX_SESSIONS` defaults to 20 and can be lowered in Railway Variables.
